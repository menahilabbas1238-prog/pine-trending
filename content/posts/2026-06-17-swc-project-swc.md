---
title: "swc-project/swc — 최신 이슈 1개 리뷰"
date: "2026-06-17"
...
- 오늘 트렌딩: **swc-project/swc** (Rust · ⭐ 34,003)
- 고른 이슈: **swc_ecma_parser : Cannot compile with --no-default-features** (open · 2026-06-16)
- 링크: https://github.com/swc-project/swc/issues/11942

## 이슈 요약 (내가 이해한 문제)

- 한 줄 요약: ### Describe the bug cargo add swc_ecma_parser --no-default-features cargo build ``` error[E0599]: no associated function or constant named `is_flow_reserved_type_name` found for struct `parser::Parser<I>` in the curren…
...
이 프로젝트는 "Rust-based platform for the Web" 쪽 문제를 건드려서 트렌딩 탔을 확률이 높고, 이슈 관리가 탄탄하면 ‘팀에 도입’까지 이어질 가능성이 큼. 현재 이 이슈의 댓글 수는 0개라서, 관심도/논의 강도를 가늠할 수 있음. 라벨은 `C-bug`가 붙어있어서 분류는 어느 정도 되어 보임. 
